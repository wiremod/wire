...
function EGP:ScaleObject( ent, obj )
	...
	if (obj.verticesindex) then -- Object has vertices
		...
		local settings = makeTable(obj, r)
		addUV(obj, settings)
		if isstring(obj.verticesindex) then settings = { [obj.verticesindex] = settings } end
		...
		if obj.w then
			obj.w = math.abs(obj.w * xMul)
		end
		if obj.h then
			obj.h = math.abs(obj.h * yMul)
		end
	end
end
...
function EGP:MoveTopLeft(ent, obj)
	...
	if obj.CanTopLeft and obj.x and obj.y and obj.w and obj.h then
		local vec, ang = LocalToWorld( Vector( obj.w / 2, obj.h / 2, 0 ), Angle(0, 0, 0), Vector( obj.x, obj.y, 0 ), Angle( 0, -obj.angle or 0, 0 ) )
		t = { x = vec.x, y = vec.y }
...
if SERVER then util.AddNetworkString("EGP_ScrHW_Request") end
if CLIENT then
	net.Receive("EGP_ScrHW_Request", function()
	...
	hook.Add("PlayerInitialSpawn","EGP_ScrHW_Request",function(ply)
		timer.Simple(1,function()
			if (ply and ply:IsValid() and ply:IsPlayer()) then
				net.Start("EGP_ScrHW_Request") net.Send(ply)
			end
		end)
	end)
