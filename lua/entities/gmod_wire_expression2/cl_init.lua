...
function wire_expression2_validate(buffer)
	...
	if not status then return buffer end

	-- load includes from root file
	local status, includes, file_buffers = wire_expression2_load_includes(include_lines, directives)
	if not status then return includes end
	...
	-- invoke compiler
	local status, script, instance = Compiler.Execute(tree, inports[3], outports[3], persists[3], dvars)
	if not status then return script end
	
	return nil, file_buffers
end
