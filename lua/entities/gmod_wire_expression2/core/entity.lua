...
e2function angle entity:angVel()
	if not validPhysics(this) then return throw("Invalid entity!") end
...
e2function vector sunDirection()
	if not IsValid(sun) then return throw("Invalid entity!") end
...
e2function number entity:mass()
	if not IsValid(this) then return throw("Invalid entity!") end
	local phys = this:GetPhysicsObject()
	if not phys:IsValid() then return throw("Invalid physics object!") end
	return phys:GetMass()
end
...
e2function void setMass(mass)
	if not validPhysics(self.entity) then return throw("Invalid entity!") end
...
e2function void entity:setMass(mass)
	if not validPhysics(this) then return throw("Invalid physics object!") end
...
e2function number entity:isWorld()
	if not isentity(this) then return throw("Invalid entity!") end
...
e2function void entity:setMaterial(string material)
	if not IsValid(this) then return throw("Invalid entity!") end
	if not isOwner(self, this) then return throw("You do not own this entity!") end
...
e2function void entity:setSkin(skinIndex)
	if not IsValid(this) then return throw("Invalid entity!") end
	if this:IsPlayer() then return throw("You cannot set the skin of a player!") end
...
e2function number entity:getBodygroups(bgrp_id)
	if IsValid(this) then return throw("Invalid entity!") end
...
e2function void entity:lockPod(lock)
	if not IsValid(this) or not this:IsVehicle() then return throw("Invalid vehicle!") end
	if not isOwner(self, this) then return throw("You do not own this entity!") end
	if lock ~= 0 then
		this:Fire("Lock", "", 0)
	else
...
e2function vector entity:aabbMin()
	if not IsValid(this) or not IsValid(this:GetPhysicsObject()) then return throw("Invalid physics object!") end
