...
--(model, default_fov)
function ENT:Setup(default_fov)
...
if CLIENT then
    ...
    function ENT:SetIsObserved(isObserved)
        ...
        if isObserved == self.IsObserved then return end
    ...
    local function CreateRTName(index)
        return "improvedrtcamera_rt_" .. tostring(index) .. "_" .. wire_rt_camera_filtering:GetString() .. "_" .. wire_rt_camera_resolution_h:GetString() .. "x" .. wire_rt_camera_resolution_w:GetString() .. (wire_rt_camera_hdr:GetInt() and "_hdr" or "_ldr")
    end
    ...
    function ENT:InitRTTexture()
        ...
        local rt = GetRenderTargetEx(CreateRTName(index), wire_rt_camera_resolution_w:GetInt(), wire_rt_camera_resolution_h:GetInt(), RT_SIZE_LITERAL, MATERIAL_RT_DEPTH_SEPARATE, filteringFlag + 256 + 32768, isHDR and CREATERENDERTARGETFLAGS_HDR or 0, isHDR and IMAGE_FORMAT_RGBA16161616 or IMAGE_FORMAT_RGB888)
        rt:Download()
        assert(rt)
...
duplicator.RegisterEntityClass("improvedrt_camera", WireLib.MakeWireEnt, "Data", "CamFOV") --[["Model",]]
duplicator.RegisterEntityClass("gmod_wire_rt_camera", WireLib.MakeWireEnt, "Data", "CamFOV") --[["Model",]]