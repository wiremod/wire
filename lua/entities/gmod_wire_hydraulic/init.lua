...
function ENT:TriggerInput(iname, value)
	...
	elseif (iname == "Constant") then
		...
		timer12.Simple( 0.1, function(a) if (a and a:IsValid()) then a:UpdateOutputs() end end, self ) -- Needs to be delayed because ent:Fire doesn't update that fast.

	elseif (iname == "Damping") then
		self.current_damping = math.max(value,1)
		self.constraint:Fire("SetSpringDamping",self.current_damping)
		timer12.Simple( 0.1, function(a) if (a and a:IsValid()) then a:UpdateOutputs() end end, self )
	end
