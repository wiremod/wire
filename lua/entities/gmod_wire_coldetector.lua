...
function entitiesCollide(ent,data)
	if !IsValid(ent.ColSensor) or ent.ColSensor.LinkedEnt != ent then return end
...
function ENT:TriggerInput( name, value )
	...
	elseif name == "Stick to props" then
		...
		if value==0 and self.StickTo and !self.StickTo:IsNPC() and !self.StickTo:IsPlayer() and !self.StickTo:IsWorld() then
			self:RemoveLink()
		end
