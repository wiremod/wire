...
function ENT:Setup(decal, range)
	self.PainterDecal = list.Contains("PaintMaterials", decal) and tostring(decal) or "Blood"
	self:SetBeamLength(math.Clamp(tonumber(range) or 0, 0, wire_painters_maxlen:GetInt()))
	self:ShowOutput(self.PainterDecal)
end

function ENT:TriggerInput(iname, value)
	if iname == "Paint" then
		if value ~= 0 then
			util.Decal(self.PainterDecal, self:GetPos(), self:GetPos() + self:GetUp() * self:GetBeamLength(), self)
		end
