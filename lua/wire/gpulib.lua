...
if CLIENT then
  ...
  local function GPULib_MemorySync()
    ...
    while true do
      ...
      -- Read block flags
      local dataFlags = net.ReadInt(8)+128
...
elseif SERVER then
  ...
  local function SendNetMessage(forcePlayer)
	if forcePlayer then
  ...
  function CACHEMGR:Flush(forcePlayer)
    ...
    for k,v in ipairs(compressInfo) do
      ...
      if v.SetOffset then
        ...
        if offsetSize == 2 then net.WriteInt(v.SetOffset, 16) messageSize = messageSize + 2 end
