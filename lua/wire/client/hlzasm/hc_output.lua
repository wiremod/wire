...
function HCOMP:Resolve(block)
  ...
  if block.Opcode then
    -- Turn unknown operations into known ones
    --[[for i=1,#block.Operands do
      if block.Operands[i].UnknownOperationByLabel then
        local label = block.Operands[i].UnknownOperationByLabel
        block.Operands[i].UnknownOperationByLabel = nil
        if label.Type == "Variable" then
          block.Operands[i].Memory = label
        elseif label.Type == "Pointer" then
          block.Operands[i].Constant = label.Expression or {{ Type = self.TOKEN.IDENT, Data = label.Name, Position = block.CurrentPosition }}
        else
          if not label.Name then self:Error("Internal error 033") end
          self:Error("Undefined label: "..label.Name,block)
          block.Operands[i].Constant = self.Settings.MagicValue
        end
      end
    end]]--
  ...
  if block.Data then
    for index,value in ipairs(block.Data) do
      if tonumber(value) then -- Data is a number
        self.WritePointer = self.WritePointer + 1
