...
Editor.NewTabOnOpen = CreateClientConVar("wire_mslcd_new_tab_on_open", "1", true, false)
...
local size = CreateClientConVar("wire_mslcd_editor_size", "800_600", true, false)
local pos = CreateClientConVar("wire_mslcd_editor_pos", "-1_-1", true, false)
...
function Editor:SaveEditorSettings()
	...
	RunConsoleCommand("wire_mslcd_editor_size", w .. "_" .. h)

	local x, y = self:GetPos()
	RunConsoleCommand("wire_mslcd_editor_pos", x .. "_" .. y)
end
...
function Editor:PerformLayout()
	...
	self.C.Tree:SetHeight(self.C.Holder:GetTall()/2)
	self.C.Properties:SetHeight(self.C.Holder:GetTall()/2)
end
...
function Editor:CreateTab(chosenfile)
	...
	editor.OnTextChanged = function(panel)
		timer.Create("mslcdautosave", 5, 1, function()
			self:AutoSave()
		end)
	end
...
function Editor:InitComponents()
	...
	self.C.Browser.Folders:SetClickOnDragHover(true)
	...
	self.C.Holder:SetBackgroundColor(Color(255, 255, 255, 255))
	...
	self.C.PropList:DockMargin(2, 0, 2, 2)
	...
	self.C.VertProps = vgui.Create("DForm", self.C.PropList)
	self.C.VertProps:Dock(FILL)
	...
	self.C.Divider:SetCookieName("wire_mslcd_editor_divider")
	...
	self.C.Poly.DoClick = function(button)
...
function Editor:InitControlPanel(frame)
	...
	NewTabOnOpen:SetConVar("wire_mslcd_new_tab_on_open")
	...
	SaveTabsOnClose:SetConVar("wire_mslcd_editor_savetabs")
	...
	OpenOldTabs:SetConVar("wire_mslcd_editor_openoldtabs")
	...
	WorldClicker:SetConVar("wire_mslcd_editor_worldclicker")
	...
	Minimap:SetConVar("wire_mslcd_editor_minimap")
	Minimap:SetText("Show minimap")
	Minimap:SizeToContents()
	Minimap:SetTooltip("Enable or disable the minimap in the editor.")
	--------------------------------------------- MSLCD TAB
	sheet = self:AddControlPanelTab("MSLCD", "icon16/computer.png", "Options for MSLCD.")
...
local wire_mslcd_editor_savetabs = CreateClientConVar("wire_mslcd_editor_savetabs", "1", true, false)
...
function Editor:InitShutdownHook()
	...
	hook.Add("ShutDown", "wire_mslcd_ShutDown" .. id, function()
		local buffer = self:GetData()
		...
		if wire_mslcd_editor_savetabs:GetBool() then
			self:SaveTabs()
		end
	end)
end
...
local wire_mslcd_editor_openoldtabs = CreateClientConVar("wire_mslcd_editor_openoldtabs", "1", true, false)
...
local wire_mslcd_editor_worldclicker = CreateClientConVar("wire_mslcd_editor_worldclicker", "0", true, false)
function Editor:SetV(bool)
	...
	self:GetParent():SetWorldClicker(wire_mslcd_editor_worldclicker:GetBool() and bool) -- Enable this on the background so we can update MSLCD's without closing the editor
end
...
function Editor:GetData()
	...
	--MSLCDSetToolInfo(self:ExtractNameFromEditor(), #data, last_data)
	return data
end
...
function Editor:Open(Line, data, forcenewtab)
	...
	hook.Run("WireMSLCDEditorOpen", self, Line, data, forcenewtab)
	self:SetV(true)
	self.C.SaE:SetText("Save and Exit")
	if data then
...
function Editor:SaveFile(Line, close, SaveAs)
	...
	if close then
		GAMEMODE:AddNotify("MSLCD saved as " .. Line .. ".", NOTIFY_GENERIC, 7)
		self:Close()
	end
end
...
function Editor:Close()
	...
	hook.Run("WireMSLCDEditorClose", self)
end
...
function Editor:Setup(nTitle, nLocation)
	...
	if wire_mslcd_editor_openoldtabs:GetBool() then
		self:OpenOldTabs()
	end
...
vgui.Register("MSLCDEditorFrame", Editor, "DFrame")
if SERVER then MsgC(Color(0, 100, 255), "MSLCD Editor loaded!\n") end