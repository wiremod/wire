...
function TOOL:RightClick( trace )
	...
	if not trace.Entity or not trace.Entity:IsValid() or trace.Entity:GetClass() ~= "gmod_wire_hudindicator" then return false end
...
function TOOL:Think()
	...
	if SERVER then
		...
		if (self.NextCheckTime or 0) < CurTime() then
			...
			if trace.Hit and trace.Entity:IsValid() and trace.Entity:GetClass() == "gmod_wire_hudindicator" and trace.Entity:GetPlayer() ~= ply then
