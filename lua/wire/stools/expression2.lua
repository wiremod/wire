...
if SERVER then
	...
	net.Receive("wire_expression2_upload", function(len, ply)
		...
		if toent.player != ply and (not hook.Run( "CanTool", ply, WireLib.dummytrace( toent ), "wire_expression2" ) or toent.player:GetInfoNum("wire_expression2_friendwrite", 0) ~= 1) then
	...
	concommand.Add("wire_expression_forcehalt", function(player, command, args)
		...
		if hook.Run( "CanTool", player, WireLib.dummytrace( E2 ), "wire_expression2" ) then --E2.player == player or E2Lib.isFriend(E2.player, player) then
	...
	concommand.Add("wire_expression_reset", function(player, command, args)
		...
		if not E2 or not E2:IsValid() or E2:GetClass() ~= "gmod_wire_expression2" then return end
		if canhas(player) then return end
		if hook.Run( "CanTool", player, WireLib.dummytrace( E2 ), "wire_expression2" ) and E2.player:GetInfoNum("wire_expression2_friendwrite", 0) == 1 then
